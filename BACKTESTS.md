...
† Computed on the **refreshed dataset** (latest game 2026-07-06), not the 2026-06-28 snapshot used by every row above — cross-row log-loss is *not* directly comparable. Same-data comparison (from `sweep_cond_draw.py`, refreshed data): conditional **beats flat draw_k=1.2 on BT2 (0.9012 vs 0.9052) and BT1 (1.1683 vs 1.1711)** but **regresses BT3 (0.9512 vs 0.9395)**. BT2-optimal (b=0.8, s=100) reaches 0.9003 but overfits 16 KO matches and wrecks BT3 (0.9576), so the balanced b=0.4 is baked in.
