...
> **Quarterfinal submission (2026-07-09 – 2026-07-11)**: main (`origin/main`, PR #4) —
> `uv run predict.py --refresh --output-dir submissions` → `submission_sync_origin-main-check_20260709.csv`
> · BT2=0.8950 (wc_recent_form, no elo_momentum)
> Note: `elo_momentum_mom5` (PR #5, BT2=**0.8791**) was evaluated for this round but
> not used for the actual submission — kept as an open experiment, not merged
...
| elo_momentum_mom5 †† | exp/elo_momentum_ablation | exp/elo_momentum | 1.1694 | **0.8791** | 0.9478 | 47.9% | 56.2% | 58.3% | cd896f4 | ablation (`sweep_elo_momentum.py`) found elo_mom5_diff **alone** beats bundling mom3+mom5 on BT1 and BT2, negligible BT3 cost → 24 features · unlike wc_recent_form's form trio, momentum windows don't need each other · new best BT2 in the TabPFN pipeline · also recovers BT3 accuracy to 58.3% (bundled version had dropped it to 56.2%) · evaluated for the Quarterfinal (2026-07-09–11) round but **not used** for the actual submission (main's wc_recent_form was used instead) · PR #5, not yet merged to main |
